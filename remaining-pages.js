...
(function () {
    ...
    // API responses can be returned as a bare array, {data: [...]}, or a
    // resource object such as {vehicles: [...]}. Normalize list responses
    // before rendering so a harmless response-shape difference never causes
    ...
    const asArray = (value, keys = []) => {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const key = keys.find(k => Array.isArray(current[k]));
    ...
    async function vehicles(c) {
        const data = asArray(await api().get("/api/vehicles"), ["vehicles"]);
    ...
    async function traffic(c) {
        const data = asArray(await api().get("/api/traffic-nodes"), ["trafficNodes", "nodes"]);
    ...
    async function signals(c) {
        const data = asArray(await api().get("/api/traffic-signals"), ["trafficSignals", "signals"]);
    ...
    async function logs(c) {
        const data = asArray(await api().get("/api/events", {limit: 200}), ["events", "logs"]);
    ...
    async function emergencyDetails(c) {
        ...
        const e=unwrap(e0), events=asArray(events0, ["events", "logs"]);
    ...
    async function vehicleDetails(c) {
        ...
        const v=unwrap(v0), t=asArray(t0, ["telemetry", "items", "results"]);
